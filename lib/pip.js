...
function generateRequirementsFile(
  ...
  if (
    ...
    requirementsMap = filterRequirementsFile(requirementsPath, targetFile, options);
    serverless.cli.log(
...
function filterRequirementsFile(source, target, options) {
  ...
  req_map = []
  for (let item of localRequirements) {
    const [localpath] = item.split(':').slice(-1)
    
    const absLocalPath = fse.realpathSync(localpath)
    const [folderName] = absLocalPath.split(path.sep).slice(-1)
    
    const containerPath = `/var/task/${folderName}`
    filteredRequirements.push(containerPath)
    req_map.push([absLocalPath, containerPath])
  }
...
function installRequirementsIfNeeded(
  ...
  req_map = generateRequirementsFile(
    fileName,
    slsReqsTxt,
    serverless,
    servicePath,
  ...
  if (options.dockerizePip) {
    options.dockerRunCmdExtraArgs = options.dockerRunCmdExtraArgs || []
    for(var req of req_map) {
      const bindPath = dockerPathForWin(getBindPath(serverless, req[0]));
      options.dockerRunCmdExtraArgs.push('-v', `${bindPath}:${req[1]}:z`);
    }
  }
